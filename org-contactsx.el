...
(defcustom org-contacts-default-tel-property "PHONE"
...
(defvar org-contacts-property-categories '("Address"
                                           "Anniversary"
                                           "Birthday"
                                           "Company"
                                           "Email"
                         ...
                         (cl-loop for contact in (org-contacts-filter
                                  ...
                                  for email = (org-contacts-strip-link
                                               (or (car (org-contacts-split-property
                                                         (or
                                                          (cdr (assoc-string org-contacts-default-email-property
                                                                             (cl-caddr contact)))
                                                          ""))) ""))
          ...
          (cl-loop for contact in (org-contacts-filter)
                   ...
                   nconc (cl-loop for email in email-list
                                  collect (org-contacts-format-email
                                           contact-name (org-contacts-strip-link
                                                         email)))))
...
(defun org-contacts-gnus-article-from-get-marker ()
  ...
  (let* ((address (org-contacts-gnus-get-name-email))
    ...
    (cl-cadar (or (org-contacts-filter
                   nil
                   nil
                   (cons org-contacts-default-email-property (concat "\\b" (regexp-quote email) "\\b")))
                  (when name
                    (org-contacts-filter
                     (concat "^" name "$")))))))
...
If not found return RETURN-VALUE or something that would ask the user."
  (or (cadr (org-contacts-gnus-get-name-email))
      return-value
      (concat "%^{" org-contacts-default-email-property "}p")))
...
(defun org-contacts-get-icon (&optional pom)
  ...
  (catch 'icon
    ...
    (when org-contacts-icon-use-alltheicons
...
Requires google-maps-el."
  ...
  (google-maps-static-show
   ...
   (cl-loop
    ...
    for addr = (cdr (assoc-string org-contacts-default-address-property (nth 2 contact)))
    if addr
     ...
     (let* ((org-contacts-buffer (find-file-noselect (car (org-contacts-files))))
            ...
            (email (save-excursion
                     (with-current-buffer org-contacts-buffer
                       (goto-char position)
                       (dolist (email-name org-contacts-email-properties email)
...
(defun org-contacts-get-properties-list (category)
...
the list `org-contacts-address-properties')."
  (let ((properties-list ""))
    (cond ((string= category "Address")
           (setq properties-list org-contacts-address-properties))
          ((string= category "Anniversary")
           (setq properties-list org-contacts-anniv-properties))
          ((string= category "Birthday")
           (setq properties-list org-contacts-birthday-property))
          ((string= category "Company")
           (setq properties-list org-contacts-company-properties))
          ((string= category "Email")
           (setq properties-list org-contacts-email-properties))
          ((string= category "Job Title")
           (setq properties-list org-contacts-job-properties))
          ((string= category "Phone")
           (setq properties-list org-contacts-tel-properties))
          (t
